...
def insert_class(class_name):
    try:
        ...
        select_query = f"INSERT Lop (TenLop) VALUES ('{class_name}')"
        cursor.execute(select_query)
...
def insert_student(name_student, maLop):
    try:
        connection = get_connection()
        cursor = connection.cursor()
        
        select_query = f"INSERT SinhVien (HoTen, MaLop) VALUES ('{name_student}', '{maLop}')"
        cursor.execute(select_query)
        connection.commit()
        print("Đã thêm thành công")
    except (Exception, pyodbc.Error) as error: 
...
def update_student(ID, name_student, maLop):
    try:
        connection = get_connection()
        cursor = connection.cursor()
        
        select_query = f"update SinhVien set HoTen = N'{name_student}', MaLop = '{maLop}' where SinhVien.ID = '{ID}'"
        cursor.execute(select_query)
...
def delete_student(ID):
    try:
        ...
        select_query = f"delete SinhVien where ID = '{ID}'"
        cursor.execute(select_query)
...
# name_student = str(input("Nhập tên sinh viên muốn thêm: "))
# maLop = int(input("Nhập mã lớp: "))
# insert_student(name_student, maLop)
# get_all_student()

# ID = int(input("Nhập mã số sinh viên muốn sửa: "))
# name_student = str(input("Sửa tên: "))
# maLop = int(input("Sửa mã lớp: "))
# update_student(ID, name_student, maLop)

ID = int(input("Nhập mã số muốn xoá: "))
delete_student(ID)
